...
public class RouteInfoManager {
    ...
    // topic和消息队列的对应关系，消息发送时根据路由表进行负载均衡
    private final HashMap<String/* topic */, List<QueueData>> topicQueueTable;
    // Broker基础信息，包含brokerName，所属集群名称，主备Broker地址
    private final HashMap<String/* brokerName */, BrokerData> brokerAddrTable;
    // broker集群信息，存储所有的Broker名称
    private final HashMap<String/* clusterName */, Set<String/* brokerName */>> clusterAddrTable;
    // broker状态信息，NameServer每次收到心跳包都会替换该消息
    private final HashMap<String/* brokerAddr */, BrokerLiveInfo> brokerLiveTable;
    // broker上的Filter Server裂变，用于类模式过滤。
    private final HashMap<String/* brokerAddr */, List<String>/* Filter Server */> filterServerTable;

    /**
     * RocketMQ 基于订阅发布机制 一个 Topic 拥有多个消息队 ，一个Broker为每一主题默
     * 认创建4个读队列4个写队列 多个 Broker 组成1个集群 BrokerName 由相同的多台 Broker
     * 组成Master-Slave架构 brokerId为0 代表 Master，大于0表示 Slave。 BrokerLivelnfo
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // 往集群添加broker名称，其中key是集群名称，value是hashSet，存储的是不同的broker名称
                ...
                // 如果broker是master，且topic配置信息发送变化或者不是首次注册，则需要去创建或者更新topic元数据
    ...
    /** 从nameServer中删除Broker信息 **/
    public void scanNotActiveBroker() {
        Iterator<Entry<String, BrokerLiveInfo>> it = this.brokerLiveTable.entrySet().iterator();
        while (it.hasNext()) {
            Entry<String, BrokerLiveInfo> next = it.next();
            long last = next.getValue().getLastUpdateTimestamp();
            // 当前时间超过BROKER_CHANNEL_EXPIRED_TIME，默认配置的120秒，就会移除
