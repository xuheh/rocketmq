...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // mqClientInstance -> 对应多个product，需要主义client_id不能重复，如果重复的话会进行覆盖。
                this.mQClientFactory = MQClientManager.getInstance().getAndCreateMQClientInstance(this.defaultMQProducer, rpcHook);
                // 将生产者注册到ClientInstance中, 方便后续调用网络请求、进行心跳检测等
     ...
     *
     * @param msg 消息内容
     * @param communicationMode 发送发送，同步，异步，单向
    ...
    private SendResult sendDefaultImpl(
        ...
        // 获取主题路由信息，只有找到主题的路由信息，才能知道发往那个broker
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        // 获取topic对应的信息，会进行本地缓存
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            // 去nameServer获取消息，并且更新到topic中
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            // 没有对应的topic时，使用当前方法进行topic的创建。DefaultMQProducer#createTopicKey
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        // 根据MessageQueue去获取Broker的地址
        String brokerAddr = this.mQClientFactory.findBrokerAddressInPublish(mq.getBrokerName());
        if (null == brokerAddr) {
            // 没有找到，根据topic名称再去nameServer搜索
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                // 如果是事务消息，则标记为PROPERTY_TRANSACTION_PREPARED
                if (tranMsg != null && Boolean.parseBoolean(tranMsg)) {
                    sysFlag |= MessageSysFlag.TRANSACTION_PREPARED_TYPE;
                }
                // 如果注册了消息发送钩子函数，则发送消息之前会执行该逻辑，通过DefaultMQProducerImpl.registerSendMessageHook注册
